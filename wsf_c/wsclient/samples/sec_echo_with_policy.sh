...
then
    ...
    if [ `uname -s` = Darwin ]
    then
        sed -e 's,WSFC_HOME,'$INST_DIR',g' -e 's,\.so,\.dylib,g' data/sec_echo_services.xml > $SERVICE_HOME/services.xml
    else
        sed 's,WSFC_HOME,'$INST_DIR',g' data/sec_echo_services.xml > $SERVICE_HOME/services.xml
    fi
