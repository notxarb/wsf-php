...
if [ `uname -s` = Darwin ]
then
    sed -e 's,WSFC_HOME,'$INST_DIR',g' -e 's,\.so,\.dylib,g' $_SCEN$_SAML/sts-client-policy.xml > $CLIENT_REPO/sts_policy.xml
else
    sed 's,WSFC_HOME,'$INST_DIR',g' $_SCEN$_SAML/sts-client-policy.xml > $CLIENT_REPO/sts_policy.xml
fi

echo "Replacing settings in Configuration files."
if [ `uname -s` = Darwin ]
then
    sed -e 's,WSFC_HOME,'$INST_DIR',g' -e 's,\.so,\.dylib,g' $_SCEN$_SAML/sts-services.xml > $SERVICE_HOME/saml_sts/services.xml
else
    sed 's,WSFC_HOME,'$INST_DIR',g' $_SCEN$_SAML/sts-services.xml > $SERVICE_HOME/saml_sts/services.xml
fi
